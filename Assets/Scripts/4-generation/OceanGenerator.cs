...
public class OceanGenerator
{
    ...
    public OceanGenerator(float randomFillPercent = 0.5f, int gridSize = 100)
    {
    ...
    public void SmoothMap()
    {
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                ...
                int surroundingReefs = GetSurroundingReefCount(x, y);

                //Use some smoothing rules to generate caves
                if (surroundingReefs > 4)
                {
                    bufferNew[x, y] = 1;
                }
                else if (surroundingReefs == 4)
                {
                    bufferNew[x, y] = bufferOld[x, y];
                }
                else if (bufferOld[x, y] == 2)
                {
                    bufferNew[x, y] = 2;
                }
                else if (bufferOld[x, y] == 3)
                {
                    bufferNew[x, y] = 3;
                }
                else
