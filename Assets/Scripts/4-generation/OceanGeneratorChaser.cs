 ...
 * Since: 2020-12
 */
public class OceanGeneratorChaser
{
    ...
    public OceanGeneratorChaser(float randomFillPercent = 0.5f, int gridSize = 20)
    {
     ...
     */
    public void RandomizeMap(int gridSize)
    {
        this.gridSize = gridSize;
        this.bufferOld = new int[gridSize, gridSize];
        this.bufferNew = new int[gridSize, gridSize];

        UnityEngine.Debug.Log("gridsize on random size" + this.gridSize);
        ...
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                if (x == 0 || x == gridSize - 1 || y == 0 || y == gridSize - 1)
                {
                    ...
                    //UnityEngine.Debug.Log("x and y on random size" + x + ", "+ y);
                    bufferOld[x, y] = 1;
                }
                else
                {
    ...
    private int GetSurroundingReefCount(int cellX, int cellY)
    {
        int wallCounter = 0;
